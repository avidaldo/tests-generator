...
Parses Moodle XML quiz files and converts them to Question/Answer dataclass instances.
Supports multi-file import with source tracking.
"""
...
def extract_cdata_content(text: str | None) -> str:
    """Extract content from potential CDATA wrapper."""
    if text is None:
        return ""
    return text
    ...
    for q_elem in root.findall("question"):
        ...
        if q_type == "category":
            ...
            for ans_elem in q_elem.findall("answer"):
                ...
                answers.append(
                    Answer(
                        text=extract_cdata_content(
                            ans_text_elem.text if ans_text_elem is not None else ""
                        ),
                        fraction=ans_elem.get("fraction", "0"),
                        feedback=extract_cdata_content(
                            ans_feedback_elem.text if ans_feedback_elem is not None else ""
                        ),
            ...
            question = Question(
                ...
                question_text=extract_cdata_content(
                    qtext_elem.text if qtext_elem is not None else ""
                ),
                general_feedback=extract_cdata_content(
                    feedback_elem.text if feedback_elem is not None else ""
                ),
                category_path=current_category_path,
                answers=answers,
                status=QuestionStatus.REVISAR,  # New questions start as "to review"
                ...
                correct_feedback=correct_fb.text if correct_fb is not None and correct_fb.text else "<p>Correcto.</p>",
                partially_correct_feedback=partial_fb.text if partial_fb is not None and partial_fb.text else "<p>Parcialmente correcto.</p>",
                incorrect_feedback=incorrect_fb.text if incorrect_fb is not None and incorrect_fb.text else "<p>Incorrecto.</p>",
...
def parse_multiple_files(xml_paths: list[Path]) -> tuple[list[Category], list[Question]]:
    ...
    Returns:
        Tuple of (all_categories, all_questions)
    """
